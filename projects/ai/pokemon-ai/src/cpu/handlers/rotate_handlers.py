...
def rla_a(cpu, opcode_info) -> int:
    """RLA A - Rotate A left through CF (no Z flag update)"""
    # Get current A value and carry flag
    a_value = cpu.get_register("A")
    carry_flag = cpu.get_flag("C")
    ...
    # Update flags - RLA does NOT update Z flag
    cpu.set_flag("Z", False)
    cpu.set_flag("N", False)
    cpu.set_flag("H", False)
    cpu.set_flag("C", old_carry_bit)
    ...
    return opcode_info["cycles"][0]  # RLA takes 4 cycles


def rra_a(cpu, opcode_info) -> int:
    """RRA A - Rotate A right through CF (no Z flag update)"""
    # Get current A value and carry flag
    a_value = cpu.get_register("A")
    carry_flag = cpu.get_flag("C")
    ...
    # Update flags - RRA does NOT update Z flag
    cpu.set_flag("Z", False)
    cpu.set_flag("N", False)
    cpu.set_flag("H", False)
    cpu.set_flag("C", old_carry_bit)
    ...
    return opcode_info["cycles"][0]  # RRA takes 4 cycles
